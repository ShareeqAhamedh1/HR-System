...
$h_a_name=getDataBack($conn,'tbl_hotel_admin','h_a_id',$h_a_id,'h_a_name');
									...
									<div class="card custom-card">
										<div class="main-content-app">
											<div class="main-content-body main-content-body-chat">
												...
												<?php 
													$sqlTicket="SELECT * FROM tbl_tickets WHERE tic_id='$tic_id' AND status=2";
													$rsTicket=$conn->query($sqlTicket);
													if($rsTicket->num_rows==1){
														?>
														<form action="backend/submit_message.php" method="POST" enctype="multipart/form-data">
															<div class="main-chat-footer">
																<nav class="nav">
																	<!-- Add Photo -->
																	<a class="nav-link" data-bs-toggle="tooltip" href="javascript:void(0);" title="Add Photo" onclick="return false;" style="pointer-events: none;">
																		<i class="fe fe-image"></i>
																	</a>
																	<input type="file" name="photo" accept="image/*" style="display: none;" id="addPhotoInput" disabled>

																	<!-- Attach a File -->
																	<a class="nav-link" data-bs-toggle="tooltip" href="javascript:void(0);" title="Attach a File" onclick="return false;" style="pointer-events: none;">
																		<i class="fe fe-paperclip"></i>
																	</a>
																	<input type="file" name="attachment" style="display: none;" id="attachFileInput" disabled>
																</nav>

																<!-- Hidden Inputs -->
																<input type="hidden" name="h_a_id" id="h_a_id" value="<?= $h_a_id ?>"> 
																<input type="hidden" name="h_s_a_id" id="h_s_a_id" value="<?= $h_s_a_id ?>">  
																<input type="hidden" name="tic_id" id="tic_id" value="<?= $tic_id ?>">     

																<!-- Message Input -->
																<input class="form-control" placeholder="You can't chat with this person" name="message" type="text" disabled>

																<!-- Submit Button -->
																<a class="main-msg-send" href="javascript:void(0);" style="pointer-events: none;">
											...
											<a href="tickets.php"><button type="button" class="btn btn-sm btn-success">Back</button></a>
										</div>
									</div>
								</div>
							</div>
