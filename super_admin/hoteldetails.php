...
?>
		...
		<div class="main-content app-content">
			<div class="container-fluid">
				...
				<!-- ROW-1 OPEN -->
				<?php
				$sqlHotel="SELECT * FROM tbl_hotel";
				...
				if ($rsHotel->num_rows>0) {
					...
					while($rowsHotel=$rsHotel->fetch_assoc()){
						$h_id=$rowsHotel['h_id'];
						$h_a_id=$rowsHotel['h_a_id'];

						$sqlAdminVerify="SELECT * FROM tbl_hotel_admin WHERE h_a_id='$h_a_id' AND verify_status=1";
						$rsAdminVerify=$conn->query($sqlAdminVerify);

						if($rsAdminVerify->num_rows>0){
							?> 
									...
									<div class="card custom-card">
										<div class="card-body position-relative">
											...
											<div class="d-flex btn-list">
												<!-- First Button -->
												<a class="btn btn-icon btn-primary btn-wave waves-effect waves-light me-2"
													onclick="openViewUSerModal(<?= $rowsHotel['h_id'] ?>)" 
												...
												<?php
												$sqlHotelEdit = "SELECT * FROM tbl_hotel_edit WHERE h_id='$h_id' AND edit_status=0";
												$rsHotelEdit = $conn->query($sqlHotelEdit);
												$rowsHotelEdit = $rsHotelEdit->fetch_assoc();

												if ($rowsHotelEdit > 0) {
												?>
													<a class="btn btn-icon btn-warning btn-wave waves-effect waves-light"
														onclick="openViewHotelModal(<?= $h_id ?>)" 
														data-bs-toggle="tooltip" 
														data-bs-original-title="View Hotel">
		...
		<script>
			...
			function openViewHotelModal(id){
				$('#editModal').modal('show');
				$('#load_edit_data').load('ajax/viewHotelDetails.php',{ h_id:id });
